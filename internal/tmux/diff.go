...
//
// Set to 32 to prevent multi-line input from being fragmented into separate hunks.
// Genuine hunk boundaries (e.g., TUI separator lines) are 80+ bytes, well above 32.
// Spurious byte-level matches within input content are typically < 10 bytes.
//
// Threshold analysis:
//   - 4 (original): Too small. Repetitive content like "The quick brown fox..."
//     creates coincidental 10+ byte matches that fragment the input hunk.
//   - 16: Borderline. Repetitive phrases can produce 16-byte coincidental matches
//     (e.g., "The quick brown " is exactly 16 bytes).
//   - 32: Safe. No practical repetitive content creates 32+ byte coincidental
//     matches, while even narrow panes (20 cols) produce genuine boundaries of
//     60+ bytes (UTF-8 box-drawing chars are 3 bytes each).
const minEqualToBreakHunk = 32
